...
showStep "Fetching Version"
value=$(<./.version)
echo -e "Version is: ${GREEN} 0.$value.0 ${RESET}" | indent
...
composer network start --card PeerAdmin@hlfv1 -n $NETWORK_NAME -V 0.$value.0 --networkAdmin admin --networkAdminEnrollSecret adminpw --file networkadmin.card
