...
echo -e "${RED} Before Installing Network, please launch docker PEER2 on HOST#2 ${RESET}" | indent
...
composer network install --card PeerAdmin@hlfv1 -a $NETWORK_NAME.bna

showStep "starting network"
echo -e "${RED} Before Starting Network, please install BNA on PEER2 / HOST#2 ${RESET}" | indent
read -p "Press any key to continue... " -n1 -s
...
composer network start --card PeerAdmin@hlfv1 -n $NETWORK_NAME -V 0.0.1 --networkAdmin admin --networkAdminEnrollSecret adminpw --file networkadmin.card
