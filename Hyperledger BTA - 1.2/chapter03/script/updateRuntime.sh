...
showStep "creating new version"
./update_version.sh
value=$(<./.version)
echo -e "New Version is: ${GREEN} 0.$value.0 ${RESET}" | indent

showStep "Deleting exiting archive..."
cd ..
rm -rf ./dist
...
composer network upgrade --card PeerAdmin@hlfv1 -n $NETWORK_NAME -V 0.$value.0
